...
# He decides to try a simple implementation of TF-IDF,
# a measure of the information content a term holds for a particular document
# in a document collection. However, he just started learning to code and
# somehow his numbers do not match what is expected. He knows you're enrolled
# in an awesome developer school, so he asks you to have a look.
...
puts tfidf("some", document3, documents) # ~ 0.

=begin ----SOLUTION----
  
The problem was that the method 'idf' was dividing the number of documents by the nubber of documents with the term, using only whole numbers.
Because whole numbers where used, the number 0.0 was returned when a number with a decimal could've been returned.

This was fixed by converting the number of documents from a whole number to an integer with a float.
